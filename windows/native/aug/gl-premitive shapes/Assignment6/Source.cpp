...
GLfloat differenceFactor = 0.533f;
...
int WINAPI WinMain(HINSTANCE currentHInstance, HINSTANCE prevHInstance, LPSTR lpszCmdLune, int iCmdShow)
{
	...
	*/
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("OpenGL Functions : Gl Premitive Shapes"),
...
void drawGrid(void)
{
	...
	{
		glVertex3f(xAxis, 0.7f, 0.0f);
		glVertex3f(xAxis, -0.7f, 0.0f);
		xAxis = xAxis + differenceFactor;
	}
...
{
	int axisX, axisY;
	GLfloat xDiff = 0.064f;// (-0.8f - differenceFactor) / 5;
	GLfloat xCord = -0.8f;
	GLfloat yCord = 0.6f;

	glBegin(GL_POINTS);
	for (axisY = 0; axisY < 5; axisY++)
	{
		xCord = -0.8f;
		for (axisX = 0; axisX < 5; axisX++)
		{
