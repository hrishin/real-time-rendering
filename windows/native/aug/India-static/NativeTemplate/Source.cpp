...
LRESULT CALLBACK WndCallbackProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 38:
			...
			glLoadIdentity();
			gluPerspective(angel, (GLfloat)glwidth / (GLfloat)glheight, 0.1f, 100.0f);
			break;
		case 40:
			angel = angel - 1;
			glMatrixMode(GL_PROJECTION);
...
void drawIndia()
{
	...
	glBegin(GL_LINES);
		...
		glColor3f(1.0, 0.6, 0.2);
		glVertex3f(1.1-.05, 0.015, 0.0);
		glVertex3f(1.3+.06, 0.015, 0.0);
		glColor3f(1.0, 1.0, 1.0);
		glVertex3f(1.1 - .05, 0.0, 0.0);
		glVertex3f(1.3 + .05, 0.0, 0.0);
		glColor3f(0.07, 0.5, 0.2);
		glVertex3f(1.1 - .05, -0.015, 0.0);
		glVertex3f(1.3 + .06, -0.015, 0.0);
	glEnd();
