...
int WINAPI WinMain(HINSTANCE currentHInstance, HINSTANCE prevHInstance, LPSTR lpszCmdLune, int iCmdShow)
{
	...
	*/
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("OpenGL Programmable Pipeline : Geometry Shader"),
	...
	if (iShaderCompiledStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "Geometry Control Shader Compilation Log : %s\n", szInfoLog);
				free(szInfoLog);
				uninitialize();
				//fclose(gpFile);  //if any error occurs write immidiately 
				exit(0);
...
void display(void)
{
	...
	glDrawArrays(GL_TRIANGLES, 0, 3); // 3 (each with its x,y,z ) vertices in triangleVertices array for 4 sides
	// unbind vao
	glBindVertexArray(0);
