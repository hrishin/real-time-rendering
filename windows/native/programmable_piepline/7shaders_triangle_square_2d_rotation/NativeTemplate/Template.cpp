...
void initialize(void)
{
	...
	*/
	...
	// Position
	glGenBuffers(1, &gVboTrianglePosition);
	glBindBuffer(GL_ARRAY_BUFFER, gVboTrianglePosition);
	...
	// Square VAO
	glGenVertexArrays(1, &gVaoSqaure);
	glBindVertexArray(gVaoSqaure);
	...
	// Color
	glGenBuffers(1, &gVboSquareColor);
	glBindBuffer(GL_ARRAY_BUFFER, gVboSquareColor);
	// move data from main memory to graphics memory
	// GL_STATIC_DRAW or GL_DYNAMIC_DRAW : How you want load data run or preloading. example game shows loadingbar and "loading" messages
...
void uninitialize(void)
{
	...
	if (gVaoSqaure)
	{
		glDeleteVertexArrays(1, &gVaoSqaure);
		gVaoSqaure = 0;
	}
...
void display(void)
{
	...
	// rotate on y axis
	modelViewMatrix = modelViewMatrix * vmath::rotate(triangleAngle, 0.0f, 1.0f, 0.0f);
	...
	modelViewMatrix = vmath::translate(1.5f, 0.0f, -6.0f);
	// rotate on all axis
	modelViewMatrix = modelViewMatrix * vmath::rotate(squareAngle, 1.0f, 1.0f, 1.0f);
	// multiply the modelview and orthographic matrix to get modelviewprojection matri
	// order is important
	modelViewProjectionMatrix = gPerspectiveProjectionMatrix * modelViewMatrix;
...
void update()
{
	triangleAngle = triangleAngle + 1.0f;
