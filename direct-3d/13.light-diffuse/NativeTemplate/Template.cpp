...
ID3D11Buffer *gpID3D11BufferCubeNormal = NULL;
...
HRESULT initialize(void)
{
	...
	ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
	vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeNormal);
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpID3D11BufferCubeNormal);
	if (FAILED(hr))
	{
	...
	//copy data into buffer
	ZeroMemory(&mappedSubSource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11BufferCubeNormal, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubSource);
	memcpy(mappedSubSource.pData, cubeNormal, sizeof(cubeNormal));
	gpID3D11DeviceContext->Unmap(gpID3D11BufferCubeNormal, NULL);
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11BufferCubeNormal, &stride, &offset);
...
void uninitialize(void)
{
	...
	if (gpID3D11BufferCubeNormal)
	{
		gpID3D11BufferCubeNormal->Release();
		gpID3D11BufferCubeNormal = NULL;
	}
