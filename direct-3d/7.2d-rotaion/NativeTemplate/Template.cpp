...
ID3D11Buffer *gpID3D11BufferTriangleVertices = NULL;
ID3D11Buffer *gpID3D11BufferTrianglColor = NULL;
ID3D11Buffer *gpID3D11BufferSquareVertices = NULL;
ID3D11Buffer *gpID3D11BufferSquarelColor = NULL;
...
int WINAPI WinMain(HINSTANCE currentHInstance, HINSTANCE prevHInstance, LPSTR lpszCmdLune, int iCmdShow)
{
	...
	*/
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("Direct3D11 : 2D rotation Square and Triangle "),
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device -> CreateBuffer(&vertexBufferDesc, NULL, &gpID3D11BufferTriangleVertices);
	if (FAILED(hr))
	{
	...
	gpID3D11DeviceContext -> Map(gpID3D11BufferTriangleVertices, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubSource);
	memcpy(mappedSubSource.pData, triangleVertices, sizeof(triangleVertices));
	gpID3D11DeviceContext -> Unmap(gpID3D11BufferTriangleVertices, NULL);
	...
	ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
	vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(triangleColor);
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device -> CreateBuffer(&vertexBufferDesc, NULL, &gpID3D11BufferTrianglColor);
	if (FAILED(hr))
	{
	...
	//copy data into buffer
	ZeroMemory(&mappedSubSource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11BufferTrianglColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubSource);
	memcpy(mappedSubSource.pData, triangleColor, sizeof(triangleColor));
	gpID3D11DeviceContext->Unmap(gpID3D11BufferTrianglColor, NULL);
	...
	ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
	vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	vertexBufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(squareVertices);
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	ZeroMemory(&vertexBufferDesc, sizeof(D3D11_BUFFER_DESC));
	vertexBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	vertexBufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(squareColor);
	vertexBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vertexBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&vertexBufferDesc, NULL, &gpID3D11BufferSquarelColor);
	if (FAILED(hr))
	{
		LOG_ERROR(gpFile, logFileName, "ID3D11Device::CreateBuffer() failed for vertex buffer \n");
		return hr;
	...
	//copy data into buffer
	ZeroMemory(&mappedSubSource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11BufferSquarelColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubSource);
	memcpy(mappedSubSource.pData, squareColor, sizeof(squareColor));
	gpID3D11DeviceContext->Unmap(gpID3D11BufferSquarelColor, NULL);
...
void display(void)
{
	...
	UINT offset = 0;
	gpID3D11DeviceContext -> IASetVertexBuffers(0, 1, &gpID3D11BufferTriangleVertices, &stride, &offset);

	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11BufferTrianglColor, &stride, &offset);
	...
	worldMatrix = XMMatrixRotationY(angle) * XMMatrixTranslation(-1.5f, 0.0f, 6.0f);
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11BufferSquarelColor, &stride, &offset);
	...
	worldMatrix = XMMatrixRotationX(angle) * XMMatrixTranslation(1.5f, 0.0f, 6.0f);
...
void uninitialize(void)
{
	...
	if (gpID3D11BufferSquarelColor)
	{
		gpID3D11BufferSquarelColor->Release();
		gpID3D11BufferSquarelColor = NULL;
	}
	...
	if (gpID3D11BufferTrianglColor)
	{
		gpID3D11BufferTrianglColor->Release();
		gpID3D11BufferTrianglColor = NULL;
	}

	if (gpID3D11BufferTriangleVertices)
	{
		gpID3D11BufferTriangleVertices -> Release();
		gpID3D11BufferTriangleVertices = NULL;
	}
