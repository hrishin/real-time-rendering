...
struct CBUFFER
{
	...
	XMVECTOR blD;
	XMVECTOR lS;
	XMVECTOR rLightPosition;
	XMVECTOR gLightPosition;
	XMVECTOR bLightPosition;
	XMVECTOR kA;
	XMVECTOR kD;
	XMVECTOR kS;
	FLOAT shininess;
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"{" \
			...
			"float4 blD;" \
			"float4 lS;" \
			"float4 rLightPosition;" \
			"float4 gLightPosition;" \
			"float4 bLightPosition;" \
			"float4 kA;" \
			"float4 kD;" \
			"float4 kS;" \
			"float shininess;" \
		...
		"{" \
			...
			"float3 rLightDirection: NORMAL1;" \
			"float3 gLightDirection: NORMAL2;" \
			"float3 bLightDirection: NORMAL3;" \
		...
		"{" \
			...
			"{" \
				...
				"{" \
					...
					"float3 rLightDirection = (float3)normalize(rLightPosition - eyeCoordinates);" \
					"float3 gLightDirection = (float3)normalize(gLightPosition - eyeCoordinates);" \
					"float3 bLightDirection = (float3)normalize(bLightPosition - eyeCoordinates);" \
					"float tnDotLdR = max(dot(trasnformedNormals, rLightDirection), 0.0);" \
					"float tnDotLdG = max(dot(trasnformedNormals, gLightDirection), 0.0);" \
					"float tnDotLdB = max(dot(trasnformedNormals, bLightDirection), 0.0);" \
					"float4 ambient = lA * kA;" \
					"float4 diffuseR = rlD * kD * tnDotLdR;" \
					"float4 diffuseG = glD * kD * tnDotLdG;" \
					"float4 diffuseB = blD * kD * tnDotLdB;" \
					"float3 reflectionVectorR = reflect(-rLightDirection, trasnformedNormals);" \
					"float3 reflectionVectorG = reflect(-gLightDirection, trasnformedNormals);" \
					"float3 reflectionVectorB = reflect(-bLightDirection, trasnformedNormals);" \
					"float3 viewerVector = -normalize((float3)eyeCoordinates);" \
					"float4 specularR = lS * kS * pow(max(dot(reflectionVectorR, viewerVector), 0.0), shininess);" \
					"float4 specularG = lS * kS * pow(max(dot(reflectionVectorG, viewerVector), 0.0), shininess);" \
					"float4 specularB = lS * kS * pow(max(dot(reflectionVectorB, viewerVector), 0.0), shininess);" \
					"output.phongLight = (ambient + diffuseR + specularR + diffuseG + specularG + diffuseB + specularB);" \
				"}" \
				"else" \
				"{" \
					"float4 eyeCoordinates = mul(viewMatrix, mul(worldMatrix, pos));" \
					"output.trasnformedNormals = mul((float3x3)mul(viewMatrix, worldMatrix), (float3)normal);" \
					"output.rLightDirection = (float3) (rLightPosition - eyeCoordinates);" \
					"output.gLightDirection = (float3) (gLightPosition - eyeCoordinates);" \
					"output.bLightDirection = (float3) (bLightPosition - eyeCoordinates);" \
		...
		"{" \
			...
			"{" \
				...
				"{" \
					...
					"float3 nomalizedLightDirectionR = normalize(input.rLightDirection);" \
					"float3 nomalizedLightDirectionG = normalize(input.gLightDirection);" \
					"float3 nomalizedLightDirectionB = normalize(input.bLightDirection);" \
					"float3 normalizedViewerVector = -normalize(input.viewerVector);" \

					"float tnDotLdR = max(dot(nomalizedTransformedNormals, nomalizedLightDirectionR), 0.0);" \
					"float tnDotLdG = max(dot(nomalizedTransformedNormals, nomalizedLightDirectionG), 0.0);" \
					"float tnDotLdB = max(dot(nomalizedTransformedNormals, nomalizedLightDirectionB), 0.0);" \
					"float4 ambient = lA * kA;" \
					"float4 diffuseR = rlD * kD * tnDotLdR;" \
					"float4 diffuseG = glD * kD * tnDotLdG;" \
					"float4 diffuseB = blD * kD * tnDotLdB;" \
					"float3 reflectionVectorR = reflect(-nomalizedLightDirectionR, nomalizedTransformedNormals);" \
					"float3 reflectionVectorG = reflect(-nomalizedLightDirectionG, nomalizedTransformedNormals);" \
					"float3 reflectionVectorB = reflect(-nomalizedLightDirectionB, nomalizedTransformedNormals);" \
					"float4 specularR = lS * kS * pow(max(dot(reflectionVectorR, normalizedViewerVector), 0.0), shininess);" \
					"float4 specularG = lS * kS * pow(max(dot(reflectionVectorG, normalizedViewerVector), 0.0), shininess);" \
					"float4 specularB = lS * kS * pow(max(dot(reflectionVectorB, normalizedViewerVector), 0.0), shininess);" \

					"return (ambient + diffuseR + specularR + diffuseG + specularG + diffuseB + specularB);" \
				"}" \
				"else" \
				"{" \
...
void display(void)
{
	...
	if (gbLight)
	{
		...
		constantBuffer.rlD = XMVectorSet(1.0f, 0.0f, 0.0f, 1.0f);
		constantBuffer.glD = XMVectorSet(0.0f, 1.0f, 0.0f, 1.0f);
		constantBuffer.blD = XMVectorSet(0.0f, 0.0f, 1.0f, 1.0f);
		constantBuffer.lS = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);
		constantBuffer.rLightPosition = XMVectorSet(0.0f, sin(rAngle) * 30.0f, cos(rAngle) * 30.0f, 1.0f);
		constantBuffer.gLightPosition = XMVectorSet(sin(gAngle) * 30.0f, 0.0f, cos(gAngle) * 30.0f, 1.0f);
		constantBuffer.bLightPosition = XMVectorSet(sin(bAngle) * 30.0f, cos(bAngle) * 30.0f, 0.0f, 1.0f);
