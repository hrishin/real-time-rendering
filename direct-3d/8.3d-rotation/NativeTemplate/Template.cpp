...
ID3D11Buffer *gpID3D11BufferPyraamidColor = NULL;
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device -> CreateBuffer(&vertexBufferDesc, NULL, &gpID3D11BufferPyraamidColor);
	if (FAILED(hr))
	{
	...
	//copy data into buffer
	ZeroMemory(&mappedSubSource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11BufferPyraamidColor, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubSource);
	memcpy(mappedSubSource.pData, pyramidColor, sizeof(pyramidColor));
	gpID3D11DeviceContext->Unmap(gpID3D11BufferPyraamidColor, NULL);
	...
	memcpy(mappedSubSource.pData, cubeColor, sizeof(cubeColor));
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11BufferPyraamidColor, &stride, &offset);
	...
	worldMatrix = XMMatrixRotationX(angle) * XMMatrixTranslation(2.0f, 0.0f, 7.0f);
...
void uninitialize(void)
{
	...
	if (gpID3D11BufferPyraamidColor)
	{
		gpID3D11BufferPyraamidColor->Release();
		gpID3D11BufferPyraamidColor = NULL;
	}
