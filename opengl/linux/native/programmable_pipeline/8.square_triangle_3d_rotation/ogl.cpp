...
const char *gpWindowTitle = "Programmable Pipeline : Multicolor Pyramid and Cube 3D rotation";
...
void Initialize(void)
{
    ...
    // move data from main memory to graphics memory
    // GL_STATIC_DRAW or GL_DYNAMIC_DRAW : How you want load data run or preloading. example game shows loadingbar and "loading" messages
    glBufferData(GL_ARRAY_BUFFER, sizeof(pyaramidColors), pyaramidColors, GL_STATIC_DRAW);
...
void Render(void)
{
	...
	modelViewMatrix = modelViewMatrix * vmath::scale(0.5f, 0.5f, 0.5f);
    // rotate on all axis
    modelViewMatrix = modelViewMatrix * vmath::rotate(cubeAngle, 1.0f, 0.0f, 0.0f);
    ...
    // draw, either by glDrawTriangles() or glDrawArrays() or glDrawElements()
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4); // 3 (each with its x,y,z ) vertices in triangleVertices array for 6 sides one by one
...
void Update()
{
    pyramidAngle = pyramidAngle + 1.0f;
    cubeAngle = cubeAngle + 1.0f;
}
