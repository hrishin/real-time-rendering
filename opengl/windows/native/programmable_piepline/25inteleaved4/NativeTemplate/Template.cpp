...
int WINAPI WinMain(HINSTANCE currentHInstance, HINSTANCE prevHInstance, LPSTR lpszCmdLune, int iCmdShow)
{
	...
	*/
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("OpenGL Programmable Pipeline : Inter leaved 1"),
...
void initialize(void)
{
	...
	*/
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, 24 * 3 * sizeof(float), &cubeVCNT);
	glBufferSubData(GL_ARRAY_BUFFER, 24 * 3 * sizeof(float), 24 * 3 * sizeof(float), &cubeVCNT);
	glBufferSubData(GL_ARRAY_BUFFER, 48 * 3 * sizeof(float), 24 * 3 * sizeof(float), &cubeVCNT);
	glBufferSubData(GL_ARRAY_BUFFER, 72 * 3 * sizeof(float), 24 * 2 * sizeof(float), &cubeVCNT);
	//position
	glVertexAttribPointer(VDG_ATTRIBUTE_VERTEX, 3, GL_FLOAT, GL_FALSE, 24 * 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(VDG_ATTRIBUTE_VERTEX);
	// COLOR
	glVertexAttribPointer(VDG_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 24 * 3 * sizeof(float), (void*)(24 * 3 * sizeof(float)));
	glEnableVertexAttribArray(VDG_ATTRIBUTE_COLOR);
	//Normals
	glVertexAttribPointer(VDG_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 24 * 3 * sizeof(float), (void*)(48 * 3 * sizeof(float)));
	glEnableVertexAttribArray(VDG_ATTRIBUTE_NORMAL);
	// texture
	glVertexAttribPointer(VDG_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 24 * 2 * sizeof(float), (void*)(72 * 3 * sizeof(float)));
