...
int WINAPI WinMain(HINSTANCE currentHInstance, HINSTANCE prevHInstance, LPSTR lpszCmdLune, int iCmdShow)
{
	...
	*/
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("OpenGL Functions : Viewports Demos"),
...
LRESULT CALLBACK WndCallbackProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 0x25:
			width = (GLfloat) gblWndWidth / 2;
			height = (GLfloat) gblWndHeight / 2;
		...
		case 'C':
			...
			glViewport(0, 0, gblWndWidth, gblWndHeight);
			break;
	...
	case WM_SIZE:
		gblWndWidth = (GLfloat) LOWORD(lParam);
		gblWndHeight = (GLfloat)HIWORD(lParam);
